...
class ClientTest(unittest.TestCase):
    ...
    def changePasswd(self):
        try:
            ...
            element = WebDriverWait(self.driver, 10, 1).until(
                EC.element_to_be_clickable((By.XPATH, "//a[contains(text(),'修改密码')]")))
            element.click()
            # 查看是否出现二级菜单
    ...
    def addUser(self):
        try:
            ...
            element = WebDriverWait(self.driver, 10, 1).until(
                EC.element_to_be_clickable((By.XPATH, "//a[contains(text(),'添加普通用户')]")))
            element.click()
        except:
            print("点击添加普通用户失败")
    ...
    def editUser(self):
        try:
            # 等待编辑用户按钮出现
            element = WebDriverWait(self.driver, 10, 1).until(
                EC.element_to_be_clickable((By.XPATH, "//a[contains(text(),'编辑用户')]")))
            element.click()
        except:
            print("点击编辑用户失败")
    ...
    def selectUser(self):
        while True:
            try:
                ...
                e = self.driver.find_element_by_xpath(
                    "//div[@title=1]/../preceding-sibling::td[1]/div/input[@type='checkbox']").is_displayed()
                print("---")
                print(e)
                break
            except:
        ...
        t = self.driver.find_element_by_xpath(
            "//div[@title=1]/../preceding-sibling::td[1]/div/input[@type='checkbox']").is_selected()
        while t == False:
            try:
                element = WebDriverWait(self.driver, 10, 1).until(EC.element_to_be_clickable(
                    (By.XPATH, "//div[@title=1]/../preceding-sibling::td[1]/div/input[@type='checkbox']")))
                element.click()
                if t == True:
                    return
                break
    ...
    def test_2_addUser(self):
        ...
        try:
            ...
            self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").send_keys(h)
            time.sleep(1)
            self.driver.find_element_by_xpath("//input[@value='保存']").click()
            time.sleep(2)
        ...
        time.sleep(1)
        self.driver.find_element_by_xpath("//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").send_keys(
            k)
        time.sleep(1)
        ...
        try:
            ...
            time.sleep(1)
            self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[3]/input[@name='descript']").send_keys(k + '1')
            time.sleep(1)
            self.driver.find_element_by_xpath("//input[@id='password']").send_keys(k + '1')
            time.sleep(1)
            self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").send_keys(k + '2')
            time.sleep(1)
    ...
    def test_3_editUser(self):
        ...
        try:
            ...
            # 确定密码
            self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").clear()
            time.sleep(1)
            self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").send_keys(d)
            time.sleep(1)
            # 联系人
            ...
            time.sleep(1)
            q4 = self.driver.find_element_by_xpath(
                "//*[@id='addgroupform']/div/div/div[5]/input[@name='repassword']").get_attribute('value')
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    test_case = [ClientTest("test_1_deleteUser"), ClientTest("test_2_addUser"), ClientTest("test_3_editUser"),
                 ClientTest("test_4_changePasswd"), ClientTest("test_5_generalUser")]
    suite.addTests(test_case)
    now = time.strftime("%Y-%m-%d %H_%M_%S")
    filePath = "./report/" + now + "result.html"
    fp = open(filePath, "wb")
    runner = HTMLTestRunnerCN.HTMLTestReportCN(stream=fp, title="公有云管理员用户测试报告", description="测试结果：")
    runner.run(suite)
    fp.close()
